...
if (input.switch_dock_m==1){
		...
		output.speed_dock_m=0.5*gripperspd;
}
...
//			printf("s m bend %d %d\n\r",input.bend_s, input.bend_m);
			...
			break;
		case 1: //unwind motors
//			printf("%d", toggle);
			output.led_m[0]=20;
			output.led_s[0]=20;
			output.direction_bend_m=1;
			output.direction_bend_s=1;
			if ((toggle==0)&(input.switch_dock_s==0)){
				toggle=1;
				}
			else if(toggle==0){;
				}		
			else if((toggle==1)&(input.switch_dock_s==1)){
				state=2;
				toggle=0;
				output.led_m[0]=0;
				output.led_m[1]=0;
				...
				if(input.switch_tension_m==0){ //if there is any tension, unwind the motor.
					output.speed_bend_s=60;
					}
				else {
					output.speed_bend_s=0;
					}
				if(input.switch_tension_s==0){ //if there is any tension, unwind the motor.
		...
		case 2: //rewind motors
			...
			if ((toggle==0)&&(input.switch_dock_s==0)){
			...
			else if ((input.switch_dock_s==1)&(toggle=1)){
					state=3;
					toggle=0;
					output.speed_bend_m=0;
					output.speed_bend_s=0;
				...
				if(input.switch_tension_m==0){ //wind until tension.
					output.speed_bend_s=0;
					}
				else {
					output.speed_bend_s=60;
					}
				if(input.switch_tension_s==0){ //wind until tension.
					output.speed_bend_m=0;
					}
				else {
					output.speed_bend_m=60;
		...
		case 4://attach both sides
			...
			if ((toggle==0)&&(input.switch_dock_s==0)){
				...
				output.direction_dock_s=0;
				output.speed_dock_s=0.5*gripperspd;
				output.direction_dock_m=0;
				output.speed_dock_m=0.5*gripperspd;
				}
				break;
