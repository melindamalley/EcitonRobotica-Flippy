...
int main(void)
{
	...
	{	
		if(MASTER==1)
		{
			...
			//example program: dock motors move when their switch is pressed, direction toggles each time
			//bend motors move when their switch is pressed, direction does not toggle
			//leds indicate switch activations

			static uint8_t dockst,dockmt,stoggle,mtoggle;
			...
			if(input.bend_s>200)
			{
				output.led_s[2]=(((int)input.bend_s-200)/6)&0xff;
			}
			else
			{
				output.led_s[2]=0;
			...
			if(input.bend_m>90)
			{
				output.led_m[2]=(((int)input.bend_m-90)/6)&0xff;
			}
			else
			{
				output.led_m[2]=0;
