...
//			printf("s m bend %d %d\n\r",input.bend_s, input.bend_m);
		...
		case 1: //unwind motors
			...
			if((toggle==1)&(input.switch_dock_s==1)){
				toggle=2;
				output.led_m[0]=30;
				output.led_m[1]=0;	
			...
			if((toggle==2)&(input.switch_dock_s==0)){
				output.speed_dock_m=0;
				toggle=3;
				}
			if (toggle==3){
				printf("%d", output.speed_bend_m);
				output.speed_dock_m=0;
				_delay_ms(10);
				}
			if((toggle==3)&(input.switch_dock_s==1)){
				output.led_s[0]=30;	
				output.led_s[1]=0;
			...
			if((toggle==4)&(input.switch_dock_s==0)){
				toggle=5;
				}
			if (toggle==5){
				output.speed_bend_s=0;
				}		
			if ((toggle==5)&&(input.switch_dock_s==1)){
				output.speed_bend_s=0;
				state=2;
				count=0;
				toggle=0;
		...
		case 2: //rewind motors
			...
			if ((input.switch_tension_s==0)&(input.switch_tension_m==0)&(toggle=1)){
				...
				break;
		case 3: //attach or detach slave side
				output.led_m[0]=20;
			if ((input.switch_dock_m==1)&&(input.switch_dock_s==1)){
				output.speed_dock_s=0;
				output.speed_dock_m=0;
				...
				break;
	case 4: //attach or detach master side
			output.led_s[0]=20;
		if ((input.switch_dock_m==1)&&(input.switch_dock_s==1)){
			output.speed_dock_s=0;
			output.speed_dock_m=0;
