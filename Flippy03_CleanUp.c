...
/*
	...
	input.accell_m[2]=z;
		...
		//i2c_write_accell( 0b11010010,0x1c,0b11100000);
	i2c_write_accell( accell_slave_addrs,0x6b,0);
	int x=((i2c_read_accell( accell_slave_addrs, 0x3b)<<8)&0xff00)+(i2c_read_accell( accell_slave_addrs, 0x3c)&0x00ff);
	int y=((i2c_read_accell( accell_slave_addrs, 0x3d)<<8)&0xff00)+(i2c_read_accell( accell_slave_addrs, 0x3e)&0x00ff);			
	int z=((i2c_read_accell( accell_slave_addrs, 0x3f)<<8)&0xff00)+(i2c_read_accell( accell_slave_addrs, 0x40)&0x00ff);
		//convert from 2's complement
    if(x>0x8000)
    {
        x=x ^ 0xffff;
